...
public class ArmorAbility extends Ability {
	...
	public static Ability matches(ItemStack itemstack) {
		...
		else if(stack.getType() == Material.LEATHER_HELMET) {
				handle = new ArmorAbility(6);
		}
		else if(stack.getType() == Material.LEATHER_LEGGINGS) {
				handle = new ArmorAbility(8);
		}
		/* GOLD */
		else if(stack.getType() == Material.GOLD_CHESTPLATE) {
				handle = new ArmorAbility(12);
		}
		else if(stack.getType() == Material.GOLD_BOOTS) {
				handle = new ArmorAbility(10);
		}
		else if(stack.getType() == Material.GOLD_HELMET) {
				handle = new ArmorAbility(8);
		}
		else if(stack.getType() == Material.GOLD_LEGGINGS) {
				handle = new ArmorAbility(11);
		}
		/* CHAIN */
		else if(stack.getType() == Material.CHAINMAIL_CHESTPLATE) {
				handle = new ArmorAbility(26);
		}
		else if(stack.getType() == Material.CHAINMAIL_BOOTS) {
				handle = new ArmorAbility(21);
		}
		else if(stack.getType() == Material.CHAINMAIL_HELMET) {
				handle = new ArmorAbility(18);
		}
		else if(stack.getType() == Material.CHAINMAIL_LEGGINGS) {
				handle = new ArmorAbility(25);
		}
		/* Iron @NERF */
		else if(stack.getType() == Material.IRON_CHESTPLATE) {
				handle = new ArmorAbility(17);
		}
		else if(stack.getType() == Material.IRON_BOOTS) {
				handle = new ArmorAbility(14);
		}
		else if(stack.getType() == Material.IRON_HELMET) {
				handle = new ArmorAbility(11);
		}
		else if(stack.getType() == Material.IRON_LEGGINGS) {
